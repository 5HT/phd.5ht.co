...
\subsection{Операційна семантика}
Традиційно індуктивні типи входять в операційну семантику систем побудованих та основі MLTT (таких як кубічні системи).
Оригінально Мартін-Льоф виразив індуктивні типи через $\Sigma$-тип, як W-типи, проте для ця система потребує Fixpoint
в мовному ядрі або пониження до системи F.
...
\tag{$W$-elimination}
\dfrac
{\splitdfrac{w: W \vdash C(w) : Type,\ x:A,\ u:B(x) \rightarrow W}{v:\Pi (y:B(x)) \rightarrow C(u(y)) \vdash c(x,u,v):C(sup(x,u))}}
...
\dfrac
{\splitdfrac{w: W \vdash C(w) : Type,\ x:A,\ u:B(x) \rightarrow W}{v:\Pi (y:B(x)) \rightarrow C(u(y)) \vdash c(x,u,v):C(sup(x,u))}}
{\splitdfrac{x:A,\ u:B(x) \rightarrow W \vdash wrec(sup(x,u),c)\ =}{c(x,u,\lambda (y:B(x)) \rightarrow wrec(u(y),c)):C(sup(x,u))}}
\end{equation}
